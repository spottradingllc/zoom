...
%define SYMLNK "/opt/spot/sentinel"
...
Application Name: %{name} Branch: %{version} salt deployment.
...
if [ -h %{SYMLNK} ]
  then
    logger -t %{name} "removing symlink for %{name}-%{version}"
    rm %{SYMLNK}
  else
    logger -t %{name} "No LN to remove for $%{name}-%{version}"
fi
...
if [ -h %{SYMLNK} ]
  then
    logger -t %{name} "link already set for %{name}-%{version}"
  else
    logger -t %{name} "setting ln for pacakge %{name}-%{version}"
    ln -s %{PKGHOME} %{SYMLNK}
fi

# ownership changes
chown -R %{USER}:"domain users" %{PKGHOME}

# ln startup script
chmod 755 %{PKGHOME}scripts/startup.sh
ln -s %{PKGHOME}scripts/startup.sh /etc/init.d/sentinel
# create chkconfig
/sbin/chkconfig sentinel on --level 35
...
logger -t %{name} "Exiting POST section for build: %{SNAME}"
%preun
# Pre Uninstall ( before your package is removed/upgraded )
logger -t %{name} "Entering PreUn section for build: %{SNAME}"
if [ -h %{SYMLNK} ]
  then
    logger -t %{name} "removing symlink"
    rm %{SYMLNK}
  else
    logger -t %{name} "Nothing to remove"
fi
