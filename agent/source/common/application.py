...
from source.action.factory import ActionFactory
...
from source.common.enum import (
    SimpleObject,
    ApplicationMode,
    PlatformType,
    AppState
)
from source.util.decorators import connected, time_this, catch_exception
from source.util.helpers import verify_attribute
from source.common.work_manager import WorkManager

if 'Linux' in platform.platform():
    from source.client.process_client import ProcessClient
else:
    from source.client.process_client import WindowsProcessClient as ProcessClient
...
class Application(object):
    ...
    def __init__(self, config, conn, queue, system, application_type):
        ...
        :type queue: source.common.unique_queue.UniqueQueue
        :type system: source.common.enum.PlatformType
        :type application_type: source.common.enum.ApplicationType
        """
        self.config = config
    ...
    def uninitialize(self):
        """
        Gracefully stop this Zookeeper session, then free any resources 
        held by the client.
        """
    ...
    def start(self, reset=True, pause=False):
        ...
        self._log.info('Error state is now {0}'.format(self._state))
        self._update_agent_node_with_app_details()
    ...
    def _update_agent_node_with_app_details(self, event=None):
        ...
        if self.zkclient.exists(self._paths['zk_agent_path'],
            ...
            data, stat = self.zkclient.get(
                self._paths['zk_agent_path'],
                watch=self._update_agent_node_with_app_details)
    ...
    def _init_work_manager(self, queue, pipe):
        """
        :rtype: source.common.work_manager.WorkManager
        """
    ...
    def _reset_after_connection_loss(self):
        ...
        """
        self._log.info('Listener callback triggered')
        map(lambda x: x.stop(), self._actions.values())  # stop actions
        self._actions.clear()
        self._predicates = []
        self._actions = self._init_actions()
