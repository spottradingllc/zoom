...
namespace Spot
{
  TestGetNodeChildrenWhenChildNodeIsEphemeral::TestGetNodeChildrenWhenChildNodeIsEphemeral( Logger::ILogger* logger, ZooKeeper::ZooKeeper* zooKeeper ) :
    m_logger( logger ),
  ...
  TestGetNodeChildrenWhenChildNodeIsEphemeral::~TestGetNodeChildrenWhenChildNodeIsEphemeral() noexcept
  {
  }


  void TestGetNodeChildrenWhenChildNodeIsEphemeral::StartUp()
  {
    try
    {
  ...
  void TestGetNodeChildrenWhenChildNodeIsEphemeral::Execute()
  {
    bool result = false;
    ...
    try
    {
      ...
      if( r1 )
      {
        ...
        bool c1 = m_zooKeeper->CreateNode( childpath1, childdata1, ZooKeeper::ZooKeeperNodeType::EPHEMERAL );
        if( c1 )
        {
          LOGGER_INFO( m_logger, boost::str( boost::format( "Child node <%s> created with data <%s>" ) % childpath1 % childdata1 ) );

          bool c2 = m_zooKeeper->CreateNode( childpath2, childdata2, ZooKeeper::ZooKeeperNodeType::EPHEMERAL );
          if( c2 )
          {
  ...
  void TestGetNodeChildrenWhenChildNodeIsEphemeral::TearDown()
  {
  ...
  void TestGetNodeChildrenWhenChildNodeIsEphemeral::OnConnected( const std::string& host )
  {
