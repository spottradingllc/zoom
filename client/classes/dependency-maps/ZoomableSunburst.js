    ...
    self.update = function(source) {
        ...
        self.arc = d3.svg.arc()
            .startAngle(function(d) {
                return Math.max(0, Math.min(2 * Math.PI, self.x(d.x)));
            })
            .endAngle(function(d) {
                return Math.max(0, Math.min(2 * Math.PI, self.x(d.x + d.dx)));
            })
            .innerRadius(function(d) {
                return Math.max(0, self.y(d.y));
            })
            .outerRadius(function(d) {
                return Math.max(0, self.y(d.y + d.dy));
            });
