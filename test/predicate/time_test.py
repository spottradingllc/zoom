...
class TimeWindowTest(TestCase):
    ...
    def test_process_met_begin(self):
        """
        Test _process_met method when setting 'begin' parameter
        """
        m = mox.Mox()
        midnight = '00:00:00'
        one = '01:00:00'
        ...
        p1._get_comparison(p1.begin).AndReturn(mid_time)  # pretend it's midnight
        m.StubOutWithMock(p2, '_get_comparison')
        p2._get_comparison(p2.begin).AndReturn(one_time)  # pretend it's one

        m.ReplayAll()

        p1._process_met()
        p2._process_met()
        # should not be 'met' if begin is 1 and it is midnight
        self.assertEqual(p1.met, False)
        # should be 'met' if begin is midnight it is one
